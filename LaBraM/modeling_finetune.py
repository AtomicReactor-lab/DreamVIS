...
class NeuralTransformer(nn.Module):
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        if x.size(1) > 570:  # 如果序列长度超过570
            # 使用平均池化调整序列长度
            x = x.transpose(1, 2)  # [B, C, L]
            x = F.adaptive_avg_pool1d(x, 570)  # 调整到570长度
            x = x.transpose(1, 2)  # [B, 570, C]
        ...
        if self.time_embed is not None:
            ...
            # 确保time_embed的长度匹配
            target_length = x.size(1) - 1  # 减去cls_token
            if time_embed.size(1) != target_length:
            ...
            # 添加归一化以稳定训练
            time_embed = F.layer_norm(time_embed, [time_embed.size(-1)])
            x[:, 1:, :] += time_embed
